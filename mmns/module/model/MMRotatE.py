...
class MMRotatE(Model):
    ...
    def __init__(self, ent_tot, rel_tot, dim=100, p_norm=1, img_emb=None,
        ...
        if margin is None or epsilon is None:
            ...
            self.rel_embeddings_range = nn.Parameter(
                torch.Tensor([(self.margin + self.epsilon) / self.dim_r]),
                requires_grad=False
            )
            nn.init.uniform_(
                tensor=self.rel_embeddings.weight.data,
                a=-self.rel_embeddings_range.item(),
                b=self.rel_embeddings_range.item()
            )
    ...
    def forward(self, data, batch_size, neg_mode='normal', neg_num=1):
        ...
        if neg_mode == "adaptive":
            ...
            neg_score_1 = self._calc(h_neg, t_neg, r_neg, mode) + self._calc(h_img_ent_emb, t_img_ent_emb, r_neg, mode)
            neg_score_2 = self._calc(h_neg, t_img_ent_emb, r_neg, mode) + self._calc(h_img_ent_emb, t_neg, r_neg, mode)
            
            selector = (neg_score_2 < neg_score_1).int()
    ...
    def _calc(self, h, t, r, mode):
        ...
        phase_relation = r / (self.rel_embeddings_range.item() / pi)
